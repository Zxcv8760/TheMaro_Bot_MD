...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  ...
  if (!text) throw `
  مرحبا اصبح زاك يدعم ChatGPT عندك اسئلة او استفسار اي شي قول\n\n❏ على سبيل المثال\n❏ ${usedPrefix + command} ترجم اكتب النص\n❏ ${usedPrefix + command} hd لتوضيح الصوره\n❏ ${usedPrefix + command} اوامر `;
  try {
        conn.sendPresenceUpdate('composing', m.chat);
        //let sistema1 = await fetch(`https://raw.githubusercontent.com/Skidy89/chat-gpt-jailbreak/main/Text.txt`).then(v => v.text());
        let sistema1 = `سوف تعمل بمثابة WhatsApp Bot الذي أنشأه بابلو.`;
      ...
      try {
        conn.sendPresenceUpdate('composing', m.chat);
        const syms1 = `سوف تعمل بمثابة WhatsApp Bot الذي أنشأه بابلو.`;
